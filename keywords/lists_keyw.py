...
def index_keyword(op : Oper, state, local : dict[str, object] | None):
    list : List | str
    index : int
...
def setindex_keyword(op : Oper, state, local : dict[str, object] | None):
...
def append_keyword(op : Oper, state, local : dict[str, object] | None):
    binarian_assert(state.is_expr, "This operation is unavailable in expressions.", state)
