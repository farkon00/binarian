...
def get_args(lexic, state) -> list[Oper]:
    ...
    for var in lexic:
        if var[0] == "[": # list parsing
            ...
            ret.append(Oper(OpIds.value_, state.current_line, values=[List(elems)]))

        elif var[0] == '"': # string parsing
            ret.append(Oper(OpIds.value_, state.current_line, values=[parse_string(var, state)]))
         ...
         var.lower().startswith("-0x"): # Hex numbers
            ...
            ret.append(Oper(OpIds.value_, state.current_line, values=[int(var, 16)]))
         ...
         var.lower().startswith("-0b"): # Binary numbers
            ...
            ret.append(Oper(OpIds.value_, state.current_line, values=[int(var, 2)]))
         ...
         var.lower().startswith("-0b"): # Octal numbers
            ...
            ret.append(Oper(OpIds.value_, state.current_line, values=[int(var, 8)]))

        elif var.isdigit() or\
         (var[0] == "-" and var[1:].isdigit()): # int parsing
            ret.append(Oper(OpIds.value_, state.current_line,
             values=[int(var)]))
